...
"""def add_precision(result_tensor, ground_truth_tensor):
  with tf.name_scope('precision'):
      with tf.name_scope('correct_prediction'):
        precision = tf.metrics.precision(ground_truth_tensor, result_tensor, weights=None)
  return precision"""
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    precision = tf.metrics.precision(ground_truth_input, final_tensor)
    ...
    #Trying to implement a confusion matrix
    LABELS = ["Floatstone",
              "Grainstone",
              "Mudstone",
              "Packstone",
    ...
    truth = np.argmax(test_ground_truth,1)

    #Create a confusion matrix on testing data
    with tf.Graph().as_default():
        cm = tf.confusion_matrix(truth, predictions)
        with tf.Session() as session:
            cm_out = session.run(cm)


    sns.heatmap(cm_out, annot=True, xticklabels=LABELS, yticklabels=LABELS, fmt='g');
    plt.xlabel('Predicted');
    plt.ylabel('True');
    plt.savefig('0.005_CombAll10000.png', bbox_inches="tight")

    # Normalize the confusion matrix so that each row sums to 1.
    #cm_out = cm_out.astype(float) / cm_out.sum(axis=1)[:, np.newaxis]

    #plt.imshow(cm_out, interpolation='nearest', cmap=plt.cm.Blues)
    ...
    #plt.savefig('0.005_20000.png', bbox_inches="tight")
